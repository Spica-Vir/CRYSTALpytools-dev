...
#-----------------------------2D/3D vector field-------------------------------#
    ...
    def read_relativistics(self, file, type, index=None, f25_file=None):
        """
        Read 2D/3D scalar / vector fields from 2c-SCF calculations, generated by
        the 'PROPS2COMP' keyword.

        .. note ::

            The standard screen output is highly recommended to add for 2D
            scalar fields, which identifies the indices of corresponding 2D
            data maps. Otherwise the ``index`` must be specified by integer or
            list of integers.
        ...
        Args:
            ...
            index (int|list): *2D only* Sequence number of headers with the '-%-MAPN'
                pattern. Useful only if the standard screen output is not
                available. Starting from 0.
            f25_file (str): *Deprecated* If specified, cover the ``file`` input.
        ...
        import numpy as np
        import pandas as pd
        from CRYSTALpytools.base.extfmt import CrgraParser, CUBEParser
        from CRYSTALpytools.relativistics import ChargeDensity, Magnetization, OrbitalCurrentDensity, SpinCurrentDensity

        # Property
        type_avail = ['DENSITY', 'MAGNETIZ', 'ORBCURDENS', 'SPICURDENS']
        type = type.upper()
        if type not in type_avail:
            raise ValueError(f"Unknown property: '{type}'.")

        # Dimension
        if f25_file is not None: file = f25_file # compatibility

        file = np.array(file, dtype=str, ndmin=1).flatten()
        if file.shape[0] == 1:
            f = open(file[0], 'r')
            header = f.readline()
            f.close()
            if '-%-' in header: # 2D plot in fort.25
                ...
                raise Exception("1 input only vailable for fort.25 or CUBE charge density.")
        elif file.shape[0] == 3:
            ndim = 3
            if type!='MAGNETIZ' and type!='ORBCURDENS':
                raise Exception("3 inputs only vailable for magnetization or orbital current density.")
        elif file.shape[0] == 9:
            ndim = 3
            if type!='SPICURDENS':
                raise Exception("9 inputs only vailable for spin current density.")
        else:
            raise Exception(f"Only 1, 3, 9 inputs are available. There are {file.shape[0]:d} inputs")
        ...
        if ndim == 2:
            ...
            if hasattr(self, 'file_name'): # Structure from output if provided
                struc = super().get_geometry()
                origin, a, b, c, _, grid, unit = CUBEParser.read_cube(file[0])
            else:
                origin, a, b, c, struc, grid, unit = CUBEParser.read_cube(file[0])
            base = units.au_to_angstrom(np.vstack([origin, a, b, c]))

            if len(file)>1:
                grid = [grid] # consistent with 2D
                for f in file[1:]:
                    o1, a1, b1, c1, s1, g1, _ = CUBEParser.read_cube(f)
                    dist = np.array([np.linalg.norm(origin-o1),
                                     np.linalg.norm(a-a1),
                                     np.linalg.norm(b-b1),
                                     np.linalg.norm(c-c1),
                                     np.linalg.norm(np.array(grid[0].shape)-np.array(g1.shape)),
                                     np.linalg.norm(struc.lattice.matrix-s1.lattice.matrix),
                                     abs(struc.num_sites-s1.num_sites)]).round(4)
                    if np.any(dist>=1e-4):
                        raise Exception("Inconsistent geometry or data grid.")

                    dist = np.linalg.norm(struc.frac_coords%1-s1.frac_coords%1)
                    if dist > 1e-4:
                        if hasattr(self, 'file_name'):
                            warn("Inconsistent geometry between property output and CUBE. Using property output's.", stacklevel=2)
                        else:
                            raise Exception("Inconsistent geometry.")
                    grid.append(g1)

        if type == 'DENSITY':
            obj = ChargeDensity(grid, base, 1, ndim, struc, unit)
            obj._set_unit('Angstrom')
        elif type == 'MAGNETIZ':
            obj = Magnetization(np.stack([grid[0], grid[1], grid[2]], axis=-1),
                                base, ndim, struc, unit)
            obj._set_unit('SI')
        elif type == 'ORBCURDENS':
            obj = OrbitalCurrentDensity(np.stack([grid[0], grid[1], grid[2]], axis=-1),
                                        base, ndim, struc, unit)
            obj._set_unit('SI')
        elif type == 'SPICURDENS':
            obj = SpinCurrentDensity(np.stack([grid[0], grid[1], grid[2]], axis=-1),
                                     np.stack([grid[3], grid[4], grid[5]], axis=-1),
                                     np.stack([grid[6], grid[7], grid[8]], axis=-1),
                                     base, ndim, struc, unit)
            obj._set_unit('SI')
