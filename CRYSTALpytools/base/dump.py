...
class ThermoHA():
    ...
    def restart(cls, inyaml):
        ...
        ha.from_frequency(edft, qpoint, freq, eigvt, symm, natom=natom, volume=vol)
...
class ThermoQHA():
    ...
    def write_thermo_freq(cls, qha, r2tot, r2avg, min_method, volume_bound):
        ...
        Args:
            qha (Quasi_harmonic): QHA object
        """
        if qha.method != 'thermo_freq' and qha.method != 'thermo_gruneisen':
    ...
    def write_thermo_gru(cls, qha):
        ...
        Args:
            qha (Quasi_harmonic): QHA object
        """
        # file should have been written by write_thermo_freq()
        file = open(qha.filename, 'r')
        df = DataFrame(file)
        line_thermo = df[df[0].str.contains(r'^\s*# QHA thermodynamic functions')].index.tolist()[0]
        line_method = df[df[0].str.contains(r'^\s*method:')].index.tolist()[0]
        df[0].loc[line_method] = 'method: thermo_gruneisen\n'
        data = ''.join([i for i in df[0].loc[0:line_thermo-1]])
        file.close(); del df
    ...
    def restart(cls, inyaml):
        ...
        for i in range(qha.ncalc):
            halist.append(
                Harmonic(filename=None, autocalc=False).from_frequency(
                    edft[i], qpoint, freq[i], eigvt, symm, natom=natom, volume=vol[i]
                )
            )
        ...
        qha.eos, e0eos = parameterize_eos(data)
        if len(qha.eos) == 0:
