...
class DLVparser():
    """
    A collection of functions to parse DL Visualize-unique files generated by
    'DLV_' keywords. Instantiation of this object is not recommaneded.
    """
    @classmethod
    def fort35(cls, filename):
        ...
        Returns:
            ...
            band (array): nBand\*nPointA\*nPointB\*nPointC\*nSpin array of 3D
                band in Hartree
            unit (str): 'a.u.'
        """
        import numpy as np
        ...
        nspin = iband.shape[0]
        band = np.zeros([nband, npt[0], npt[1], npt[2], nspin], dtype=float)
        ...
        na = int((band.shape[1]-1)/2+1)
        nb = int((band.shape[2]-1)/2+1)
        nc = int((band.shape[3]-1)/2+1)
        bandnew = np.zeros([nband, na, nb, nc, nspin])
        idx = np.where(band<9999)
        bandnew[:, idx[1]%na, idx[2]%nb, idx[3]%nc, :] = band[:, idx[1], idx[2], idx[3], :]
        del band, idx
